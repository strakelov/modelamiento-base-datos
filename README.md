# Actividad Sumativa Semana 3 - Modelamiento Base de Datos
- **Nombre completo:** Gabriel Alejandro Gomez Pizarro
- **Carrera:** Analista Programador Computacional
- **Fecha de entrega:** 31/08/2026
- **Sección:** 004A
- **Sede:** Campus Virtual

---

## Descripción general del sistema

Este repositorio contiene el desarrollo de la Actividad Sumativa de la Semana 3 para la asignatura **Modelamiento de Bases de Datos** (Compañía HiLogic Solutions).

---

## Contenido del Repositorio
* `Exp1_S3_Gabriel.docx`: Documento con las respuestas y capturas del modelo entidad-relación en notación Barker y Bachman.
* `modelo_base.rar`: Proyecto comprimido generado desde Oracle SQL Developer Data Modeler (archivo `.dmd` y su carpeta contenedora).
